...
func closure() func() { //funcao closure, sem parâmetros, que returna uma funcão sem parâmetros 
	x := 10 //variável x no primeiro contexto
	var funcao = func() { //a variável funcao recebe a funcao sem parâmetros func, que irá imprimir a variável x
		fmt.Println(x) 
	}
